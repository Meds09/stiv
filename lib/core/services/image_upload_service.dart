...
class ImageUploadService {
  ...
  /// 
  /// [image] - The image file to upload.
  /// [userId] - The ID of the user (used for naming the file).
  Future<String> uploadProfileImage(XFile image, String userId) async {
    try {
      ...
      XFile? result = await FlutterImageCompress.compressAndGetFile(
        ...
        tempPath,
        quality: 85, // Good quality/size balance
        minWidth: 512, // Smaller for profile images
        minHeight: 512,
      );
      ...
      // 2. Define storage reference
      final String storagePath = 'users/$userId/${millisecondsSinceEpoch()}.jpg';
      final Reference ref = _storage.ref().child(storagePath);
